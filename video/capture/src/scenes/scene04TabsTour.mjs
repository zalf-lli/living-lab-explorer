...
export async function sceneTabsTour(page, ctx) {
  const t0 = Date.now()
  const mark = (label) => console.log(`[scene-04 timing] +${((Date.now() - t0) / 1000).toFixed(2)}s ${label}`)
  ...
  await ctx.annotate(layerTabStrip(page), 'tabs', { durationMs: 3400, place: 'below' })
  ...
  await ctx.annotate(statPanel(page), 'kpis', { durationMs: 3200, place: 'below' })
  ...
  await ctx.annotate(mapInfoButton(page), 'citation', { durationMs: 3600, place: 'right' })
  await hoverHuman(page, mapInfoButton(page), { steps: 20, settleMs: 500 })
  await page.waitForTimeout(2600)
  ...
  if (economicBox) {
    await ctx.annotate(economicBox, 'landPrice', { durationMs: 4000, place: 'right' })
    ...
    const sawTooltip = await sweepForTooltip(page, points, { pauseMs: 1100 })
  ...
  await ctx.annotate(protectedAreasToggle(page), 'protectedAreas', { durationMs: 3800, place: 'below' })
  await page.waitForTimeout(500)
  ...
  if (landscapeBox) {
    ...
    const opened = await clickUntilPopupOpens(page, points, { holdMs: 2600 })
