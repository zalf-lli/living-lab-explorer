...
- The video opens with a title card that is drawn by Remotion rather than captured, so it has no
  manifest entry — see `INTRO_FRAMES` / `INTRO` in `video/remotion/src/scenes.ts`. Its QR code is
  generated by `video/remotion/scripts/generate-qr.mjs`, which keeps the encoded URL in source and
  decodes the result back to verify it before the file is written.
