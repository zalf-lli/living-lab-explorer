...
| Format check | `cd app && npm run format:check` | **exit 1** — 16 files flagged, all due to the pre-existing Windows-checkout CRLF-vs-Prettier's-default-LF condition documented identically in `10-01-SUMMARY.md`, `10-02-SUMMARY.md`, `10-03-SUMMARY.md`, `10-04-SUMMARY.md`, `10-05-SUMMARY.md`, and this phase's `deferred-items.md`. Verified `LLDetail.jsx` specifically: `diff <(tr -d '\r' < src/pages/LLDetail.jsx) <(tr -d '\r' < $(npx prettier --write --config .prettierrc.json <scratch-copy>))` shows **zero content differences** — the file is fully Prettier-compliant modulo line endings. Out of scope per the deviation rules' scope boundary (pre-existing, repo-wide, unrelated to this plan's files). |
