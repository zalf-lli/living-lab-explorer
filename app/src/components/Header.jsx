...
export function Header({ lls }) {
  ...
  // Below 1024 the header stacks into two rows (brand + language, then the pill strip). At
  // 768-1023 it used to make *three* rows — brand, pills, language — and eat 190px of a
  // 1024px tablet screen, because a wrapping row-direction flex container gave the pill strip
  // and the language toggle a line each. `isMobile` additionally shrinks the wordmark; the
  // stacking itself is not phone-specific.
  ...
  return (
    // On a phone the five Living Lab pills wrapped to four rows and the header ate 235px of
    // an 812px screen before a single word of content appeared. Two changes fix that: the
    // header becomes a two-row grid (brand + language, then the pills), and the pills become
    // one horizontally-scrollable strip instead of a wrapping block.
    <div
      style={{
        flexShrink: 0,
        background: C.bg,
        ...
        gap: stacked ? 8 : 12,
        // `flex-wrap: wrap` must not be combined with `flex-direction: column` here. A
        // multi-line column flex container sizes each flex line to its items' max-content
        // width, so `align-items: stretch` then stretched both rows to the pill strip's
        // 824px natural width and the whole header overflowed a 375px screen sideways.
    ...
    >
      ...
      >
        ...
        >
          ...
          />
          <div style={{ width: 1, height: 28, background: 'rgba(30,82,56,0.25)', flexShrink: 0 }} />
          <div
            style={{
              color: C.badgeBg,
        ...
        {/* Stacked, the language toggle rides on the brand row so the pill strip below gets
            the full width to itself. */}
        {stacked ? <LanguageToggle activeLang={activeLang} i18n={i18n} t={t} isNarrow /> : null}
      </div>
      ...
      >
              ...
              <button
                ...
                onClick={() => {
                  if (compareSlug && activeSlug) {
                    ...
                    // so anything added later rides along untouched. (?layout used to be the
                    // param this protected; the layout is derived from the viewport now.)
                ...
                style={{
                  padding: isNarrow ? '9px 14px 9px 8px' : '5px 12px 5px 6px',
        ...
        {stacked ? null : <LanguageToggle activeLang={activeLang} i18n={i18n} t={t} isNarrow={isNarrow} />}
      </div>
    </div>
  )
}

function LanguageToggle({ activeLang, i18n, t, isNarrow }) {
  return (
    <div
      role="group"
      style={{
        ...
        return (
          <button
            ...
            style={{
              ...
              cursor: 'pointer',
              borderRadius: 999,
              // 27px tall was well under the 44px touch minimum; widened and heightened on
              // narrow/touch viewports only, so the desktop header keeps its compact chip.
              padding: isNarrow ? '10px 14px' : '6px 10px',
